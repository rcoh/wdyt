...
/// Static page styles, embedded in the binary and served from `/assets/wdyt.css`.
pub(crate) const STYLESHEET: &str = include_str!("../assets/wdyt.css");

/// Syntax highlighting colors vary by session and cascade from the document root.
fn theme_style(colors: &ThemeColors) -> String {
    format!(
        "--code-bg: {}; --code-fg: {}; --code-gutter: {}; \
         --code-border: {}; --code-muted: {}",
        colors.background, colors.foreground, colors.gutter, colors.border, colors.muted,
    )
}

fn stylesheet(colors: &ThemeColors) -> String {
    format!(":root {{ {} }}\n{STYLESHEET}", theme_style(colors))
}
...
pub async fn shell(
    page: Page,
    colors: &ThemeColors,
    /// The page's server-rendered subheader and `<main>` content.
    ...
    view! {
        <!DOCTYPE html>
        <html lang="en" style=(theme_style(colors))>
            <head>
                <meta charset="utf-8">
                <meta name="viewport" content="width=device-width, initial-scale=1">
                <title>(&title) " · wdyt"</title>
                <link rel="stylesheet" href="/assets/wdyt.css">
            </head>
...
/// Test-visible: returns the static stylesheet served from the asset route.
pub fn stylesheet_asset_for_test() -> &'static str {
    STYLESHEET
}
...
mod tests {
    ...
    fn the_chrome_does_not_change_with_the_theme() {
        // The linked stylesheet is static; only the root's inline variables
        // differ between a dark and a light syntax theme.
        assert_eq!(STYLESHEET, include_str!("../assets/wdyt.css"));
        assert_ne!(
            theme_style(&theme_colors(theme("Nord"))),
            theme_style(&theme_colors(theme("InspiredGitHub")))
        );
    }
